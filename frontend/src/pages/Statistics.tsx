...
export default function Statistics() {
  ...
  useEffect(() => {
    ...
    if (!(pb.authStore.model?.moderator as boolean ?? false)) {
      navigate('/home')
    }
  ...
  async function getMonthHistory() {
    try {
      const start = moment().startOf('month').subtract(offset, 'month').subtract(720, 'minute')
      const end = moment().endOf('month').subtract(offset, 'month').add(720, 'minute')
      console.log(`created>"${formatTime(start)}"&&created<"${formatTime(end)}"`)
      ...
      const userHours = new Map<string, WTask[]>()
